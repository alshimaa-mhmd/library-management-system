...
export default async function BorrowedBook({borrowedBooks}){
    ...
    const isDue = await borrowedBooks.due_date == getCurrentDate();
    ...
    return (
        <div className="flex flex-col items-start justify-normal md:justify-center gap-8">
            ...
            <div className="flex gap-6 flex-wrap w-full items-start justify-normal">
            { borrowedBooks?.reverse().map((borrowedBook) => (
            <div key={borrowedBook.id} className="h-[490px] w-[280px]  rounded-[16px] p-5 bg-[#12141D] flex flex-col items-start justify-between">
                ...
                >
                    ...
                    <img src={borrowedBook.books.cover_url} alt='book cover' className="w-[160px] h-[200px]" />
                    </div>
                </div>
                <p className="text-wrap text-[20px] font-semibold leading-6">{borrowedBook.books.title}</p>
                <i className="text-[16px] font-[400] leading-5 text-[#D6E0FF]">{borrowedBook.books.category}</i>
                <div className='flex items-center justify-center gap-1'>
                    <Image src={book} alt='book icon'  />
                    <p className="text-[16px] font-[400] leading-6 text-[#D6E0FF]">Borrowed on <span className='text-[#EED1AC]'>{borrowedBook.borrowed_at}</span> </p>
                </div>
                <div className='flex items-center justify-center gap-1'>
